...
int watcher_init(
    ...
    if (signal_fd == -1) {
        fprintf(stderr, "error: invalid signal file descriptor passed to watcher\n");
        return 1;
    }
    ...
    for (unsigned short i = 0; i < block_count; ++i) {
        ...
        if (block_fd == -1) {
            fprintf(stderr, "error: invalid block file descriptors passed to watcher\n");
            return 1;
        }
...
int watcher_poll(watcher* watcher, const int timeout_ms) {
    ...
    if (event_count == -1 && errno != EINTR) {
        fprintf(stderr, "error: watcher could not poll blocks\n");
        return 1;
    }
