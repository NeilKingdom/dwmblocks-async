...
bool status_update(status *const status) {
    strncpy(status->previous, status->current, LEN(status->current));
    status->current[0] = '\0';
    ...
    for (unsigned short i = 0; i < status->block_count; ++i) {
        ...
        if (strlen(block->output) > 0) {
            ...
            strncat(status->current, DELIMITER, LEN(DELIMITER));
#else
            ...
            if (block->signal > 0) {
                ...
                strncat(status->current, signal, LEN(signal));
            }
#endif

            strncat(status->current, block->icon, LEN(block->output));
            strncat(status->current, block->output, LEN(block->output));
        }
    }
