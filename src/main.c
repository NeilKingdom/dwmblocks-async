...
static int deinit_blocks(block *const blocks, const unsigned short block_count) {
    for (unsigned short i = 0; i < block_count; ++i) {
        block *const block = &blocks[i];
...
static int event_loop(
    ...
    while (is_alive) {
        ...
        for (unsigned short i = 0; i < watcher.active_block_count; ++i) {
            block_update(&blocks[watcher.active_blocks[i]]);
        }
...
#undef BLOCK
    ...
    if (event_loop(blocks, block_count, cli_args.is_debug_mode,
            connection, &signal_handler) != 0) {
        status = 1;
    }
