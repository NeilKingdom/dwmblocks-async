...
int signal_handler_init(signal_handler *const handler) {
    ...
    if (handler->fd == -1) {
        fprintf(stderr, "error: could not create file descriptor for signals\n");
        return 1;
    }
...
int signal_handler_deinit(signal_handler *const handler) {
    if (close(handler->fd) != 0) {
        fprintf(stderr, "error: could not close signal file descriptor\n");
        return 1;
    }
...
int signal_handler_process(signal_handler *const handler, timer *const timer) {
    ...
    if (bytes_read == -1) {
        fprintf(stderr, "error: could not read info of incoming signal");
        return 1;
    }

    const int signal = info.ssi_signo;
    switch (signal) {
        case TIMER_SIGNAL:
