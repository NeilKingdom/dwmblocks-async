...
int block_deinit(block *const block) {
    ...
    if (status != 0) {
        fprintf(
            stderr,
            "error: could not close \"%s\" block's pipe\n",
            block->command
        );
        return 1;
    }
...
int block_execute(block *const block, const uint8_t button) {
    ...
    if (block->fork_pid == -1) {
        fprintf(
            stderr,
            "error: could not create a subprocess for \"%s\" block\n",
            block->command
        );
        return 1;
    }
    ...
    if (block->fork_pid == 0) {
        ...
        if (button != 0) {
            char button_str[4];
            snprintf(button_str, LEN(button_str), "%hhu", button);
            status |= setenv("BLOCK_BUTTON", button_str, 1);
        }

        const char null = '\0';
        if (status != 0) {
            write(write_fd, &null, sizeof(null));
            exit(EXIT_FAILURE);
        }
        ...
        // Ensure null-termination since fgets() will leave buffer untouched on no output
        char buffer[LEN(block->output)] = {[0] = null};
        fgets(buffer, LEN(buffer), file);
        ...
        const size_t output_size = truncate_utf8_string(buffer, LEN(buffer), MAX_BLOCK_OUTPUT_LENGTH);
        write(write_fd, buffer, output_size);
...
int block_update(block *const block) {
    ...
    const ssize_t bytes_read = read(block->pipe[READ_END], buffer, LEN(buffer));
    if (bytes_read == -1) {
        fprintf(
            stderr,
    ...
    strncpy(block->output, buffer, LEN(buffer));
    return 0;
}
