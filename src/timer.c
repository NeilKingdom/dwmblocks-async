...
static unsigned int compute_tick(const block *const blocks, const unsigned short block_count) {
    unsigned int tick = 0;
...
static unsigned int compute_reset_value(const block *const blocks, const unsigned short block_count) {
...
int timer_arm(timer *const timer) {
    ...
    if (errno != 0) {
        fprintf(stderr, "error: could not arm timer\n");
        return 1;
    }
